...
namespace ytst {
	void HttpClient::cb(struct ev_loop *loop, ev_io *watcher, int revents) {
	...
	HttpClient::HttpClient(struct ev_loop *loop, int s) : loop(loop), sfd(s) {
		fcntl(s, F_SETFL, fcntl(s, F_GETFL, 0) | O_NONBLOCK);
		printf("Got connection\n");

		io.data = (void *)this;
		ev_io_init(&io, HttpClient::cb, s, EV_READ);
		ev_io_start(loop, &io);
	}
}
